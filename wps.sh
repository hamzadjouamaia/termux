    ...
    def iw_scanner(self) -> Dict[int, dict]:
        ...
        for n, network in network_list_items:
            ...
            if (network['BSSID'], network['ESSID']) in self.stored:
                ...
                proc = subprocess.Popen('play-audio $PREFIX/bin/sonar.mp3', shell=True)
            else:
                print(line)
...
EOF
chmod 777 $PREFIX/bin/oneshot.py $PREFIX/bin/vulnwsc.txt $PREFIX/bin/wps
pkg update
pkg upgrade -y
